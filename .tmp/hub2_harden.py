...
s = p.read_text()
old = '''        let observer_mutation = Request::builder()
            .method("POST")
...
'''
if s.count(old) != 1:
    raise SystemExit(f'authorization test anchor count={s.count(old)}')
p.write_text(s.replace(old, new, 1))

adr = '''# ADR 0019: Static control-plane principals and authorization
...
'''
Path('docs/adr/0019-static-principal-authorization.md').write_text(adr)
